...
use crate::{
    constants::{
        PUBLICKEY_SERIALIZED_LENGTH, SECRETKEY_NONCE_LENGTH, SECRETKEY_SCALAR_LENGTH,
        SECRETKEY_SEED_LENGTH, SHA512_LENGTH, SIGNATURE_SERIALIZED_LENGTH,
    },
    edwards::{CompressedY, EdwardsPoint},
    hash::Sha512,
    scalar::Scalar,
    Error, Result,
};
...
pub struct SecretKey {
    ...
    pub nonce: [u8; SECRETKEY_NONCE_LENGTH],
}
...
impl ed25519::signature::Signer<ed25519::Signature> for Keypair {
    fn try_sign(
        &self,
        msg: &[u8],
    ) -> core::result::Result<ed25519::Signature, ed25519::signature::Error> {
...
impl From<&[u8; SECRETKEY_SEED_LENGTH]> for SecretKey {
    fn from(seed: &[u8; SECRETKEY_SEED_LENGTH]) -> SecretKey {
        let hash = Sha512::new().updated(seed).finalize();
...
impl Signature {
    pub fn to_bytes(&self) -> [u8; SIGNATURE_SERIALIZED_LENGTH] {
        let mut signature_bytes: [u8; SIGNATURE_SERIALIZED_LENGTH] =
            [0u8; SIGNATURE_SERIALIZED_LENGTH];
...
mod tests {
    ...
    fn test_decompression() {
        ...
        // ];
        let mut seed: [u8; 32] =
            hex!("35b00776179a785034ff4c8288005df4acaf0b33aa1210adec308247253e6c65");
    ...
    fn test_signature() {
        ...
        let seed: [u8; 32] = [
            0x35, 0xb3, 0x07, 0x76, 0x17, 0x9a, 0x78, 0x58, 0x34, 0xf0, 0x4c, 0x82, 0x88, 0x59,
            0x5d, 0xf4, 0xac, 0xa1, 0x0b, 0x33, 0xaa, 0x12, 0x10, 0xad, 0xec, 0x3e, 0x82, 0x47,
            0x25, 0x3e, 0x6c, 0x65,
        ];
        ...
        let R_expected = [
            0xec, 0x97, 0x27, 0x40, 0x07, 0xe7, 0x08, 0xc6, 0xd1, 0xee, 0xd6, 0x01, 0x9f, 0x5d,
            0x0f, 0xcb, 0xe1, 0x8a, 0x67, 0x70, 0x8d, 0x17, 0x92, 0x4b, 0x95, 0xdb, 0x7e, 0x35,
            0xcc, 0xaa, 0x06, 0x3a,
        ];

        let S_expected = [
            0xb8, 0x64, 0x8c, 0x9b, 0xf5, 0x48, 0xb0, 0x09, 0x90, 0x6f, 0xa1, 0x31, 0x09, 0x0f,
            0xfe, 0x85, 0xa1, 0x7e, 0x89, 0x99, 0xb8, 0xc4, 0x2c, 0x97, 0x32, 0xf9, 0xa6, 0x44,
            0x2a, 0x17, 0xbc, 0x09,
        ];
